...
pub fn render_quality_intro(width: u16, height: u16) -> String {
    crate::tui::render_to_string(width, height, draw_quality_intro_frame)
}

pub(crate) fn quality_activity_lines(activity: &[String]) -> Vec<String> {
...
pub(crate) fn quality_dimension_lines() -> Vec<String> {
    QUALITY_DIMENSIONS
        .iter()
...
pub(crate) fn draw_quality_grading_frame(frame: &mut ratatui::Frame<'_>, activity: &[String]) {
...
pub(crate) fn draw_quality_intro_frame(frame: &mut ratatui::Frame<'_>) {
    let chunks = Layout::default()
        .direction(Direction::Vertical)
        .constraints([
            Constraint::Length(3),
