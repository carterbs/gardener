//! Linter: ensures QUALITY_DIMENSIONS in tui.rs stays in sync with the
...
/// Extract dimension keys and descriptions from QUALITY_DIMENSIONS in tui.rs.
///
/// The source format is:
/// ```ignore
...
fn quality_dimensions_match_runner_and_tui() {
    ...
    for dim in &runner_dims {
        if !tui_dims.contains_key(dim) {
            violations.push(format!(
                "dimension '{dim}' exists in quality_assessment_runner.rs but missing from QUALITY_DIMENSIONS in tui.rs"
            ));
        }
    }
    ...
    for dim in tui_dims.keys() {
        if !runner_dims.contains(dim) {
            violations.push(format!(
                "dimension '{dim}' exists in QUALITY_DIMENSIONS in tui.rs but missing from quality_assessment_runner.rs"
            ));
        }
    }
