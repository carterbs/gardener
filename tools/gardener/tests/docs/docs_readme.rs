...
fn docs_readme_documents_test_domain_start_point() {
    ...
    let start_here = "cargo test -p gardener --test docs_integration";
    assert!(
    ...
    assert!(
        readme.contains("`tools/gardener/tests/docs_integration.rs`"),
        "docs/README.md should include a verification-harness harness entry file"
    );
    assert!(
        readme.contains("`tools/gardener/tests/cli_integration.rs`"),
        "docs/README.md should include a verification-harness CLI harness entry file"
    );
    assert!(
