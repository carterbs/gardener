...
#[test]
fn workspace_clippy_lint_configuration_enables_trivially_copy_pass_by_ref_warn() {
    let mut manifest_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    manifest_path.pop();
    manifest_path.pop();
    manifest_path.push("Cargo.toml");
    ...
    let level = manifest
        ...
        .and_then(|workspace| workspace.get("lints"))
        .and_then(|lints| lints.get("clippy"))
        .and_then(|clippy| clippy.get("trivially_copy_pass_by_ref"))
        .and_then(Value::as_str)
        .expect("workspace.lints.clippy.trivially_copy_pass_by_ref is not configured");
