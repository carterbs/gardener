...
impl PhaseRuntime {
    pub fn init(binary_name: &str) -> Result<Self, GardenerError> {
        crate::logging::append_run_log(
            "info",
            "phase_cli.init",
            serde_json::json!({ "binary": binary_name }),
        );
...
pub fn resolve_worktree_from_pr(
    ...
    step(
        binary_name,
        "LOOKUP",
        &format!("fetching PR #{pr} metadata"),
    );
    ...
    step(
        binary_name,
        "LOOKUP",
        &format!("branch={branch} state={}", pr_view.state),
    );
    ...
    if pr_view.state.eq_ignore_ascii_case("CLOSED") {
        return Err(GardenerError::Process(
            "PR is closed (not merged)".to_string(),
        ));
    }
...
pub fn resolve_worktree_from_branch(
    ...
    if let Some(wt) = existing_wt {
        step(
            binary_name,
            "WORKTREE",
            &format!("reusing {}", wt.path.display()),
        );
    ...
    let wt_dir = scope
        .working_dir
        .join(".worktrees")
        .join(format!("{binary_name}-{pr}"));
    step(
        binary_name,
        "WORKTREE",
        &format!("creating at {}", wt_dir.display()),
    );
