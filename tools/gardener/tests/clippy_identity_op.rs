...
#[test]
fn workspace_clippy_lint_configuration_enables_unnecessary_filter_map_warn() {
    let mut manifest_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    manifest_path.pop();
    manifest_path.pop();
    manifest_path.push("Cargo.toml");
    ...
    let level = manifest
        ...
        .and_then(|workspace| workspace.get("lints"))
        .and_then(|lints| lints.get("clippy"))
        .and_then(|clippy| clippy.get("unnecessary_filter_map"))
        .and_then(Value::as_str)
        .expect("workspace.lints.clippy.unnecessary_filter_map is not configured");
