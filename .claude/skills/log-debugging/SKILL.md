...
## OTEL log utility to use

Use the structured OTEL utility instead of manual `jq` pipelines:

```bash
export OTEL_LOGS_BIN=${OTEL_LOGS_BIN:-"cargo run --manifest-path tools/gardener/Cargo.toml --bin log-query --"}
export LOG_PATH=${GARDENER_LOG_PATH:-$HOME/.gardener/otel-logs.jsonl}
```

If/when an `otel-logs` binary is available, set:

```bash
export OTEL_LOGS_BIN="otel-logs"
```

and keep the remaining commands unchanged.

## Log sanity and discovery

- Show top-level summary (events, runs, workers):
  - `$OTEL_LOGS_BIN --log-path "$LOG_PATH" stats`
- Fetch matching events:
  - `$OTEL_LOGS_BIN --log-path "$LOG_PATH" events --run-id <RUN_ID> --limit 100`
- Build a compact timeline:
  - `$OTEL_LOGS_BIN --log-path "$LOG_PATH" timeline --run-id <RUN_ID> --limit 200`

## Failure-to-logs workflow

1. Find likely failures in the current log stream:
  - `$OTEL_LOGS_BIN --log-path "$LOG_PATH" events --contains '"terminal":"failure"' --limit 50`
  - `$OTEL_LOGS_BIN --log-path "$LOG_PATH" events --event-type "agent.turn.finished" --limit 50`
2. Capture the run id from a failure line and assign:
  - `RUN=...`
3. Reconstruct full context for that run:
  - `$OTEL_LOGS_BIN --log-path "$LOG_PATH" timeline --run-id "$RUN"`
4. Pull raw rows when payload fields are needed:
  - `$OTEL_LOGS_BIN --log-path "$LOG_PATH" events --run-id "$RUN" --raw --contains '"terminal"' --limit 200 | jq -R 'fromjson? // empty | "\(.logRecord.timeUnixNano) \(.event_type) run=\(.payload.run_id // \"\") worker=\(.payload.worker_id // \"\") terminal=\(.payload.terminal // \"\") cmd=\(.payload.command // \"\")"'`
...
- Find run candidates from workdir path:
  - `WORKDIR="/Users/bradcarter/Documents/Dev/gardener/.worktrees/worker-1..."`
  - `$OTEL_LOGS_BIN --log-path "$LOG_PATH" events --contains "$WORKDIR" --raw | head -n 200`
- Resolve commit context from a known run id:
  - `RUN=...`
  - `gitroot=$($OTEL_LOGS_BIN --log-path "$LOG_PATH" events --run-id "$RUN" --contains '"run.working_dir"' --raw | jq -R 'fromjson? // empty | .logRecord.attributes[]? | select(.key=="run.working_dir") | .value.stringValue' | head -n 1)`
...
- adapter parse issues:
  - `$OTEL_LOGS_BIN --log-path "$LOG_PATH" events --event-type "stdout_non_json"`
- process spawn failures:
  - `$OTEL_LOGS_BIN --log-path "$LOG_PATH" events --event-type "process_spawn"`
- terminal transitions:
  - `$OTEL_LOGS_BIN --log-path "$LOG_PATH" events --event-type "terminal_result"`
...
RUN=...
$OTEL_LOGS_BIN --log-path "$LOG_PATH" timeline --run-id "$RUN"
$OTEL_LOGS_BIN --log-path "$LOG_PATH" events --run-id "$RUN" --contains '"terminal":"failure"' --raw --limit 50 \
