...
#[test]
fn workspace_clippy_lint_configuration_enables_redundant_closure_for_method_calls_warn() {
    let mut manifest_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    manifest_path.pop();
    manifest_path.pop();
    manifest_path.push("Cargo.toml");
    ...
    let level = manifest
        ...
        .and_then(|workspace| workspace.get("lints"))
        .and_then(|lints| lints.get("clippy"))
        .and_then(|clippy| clippy.get("redundant_closure_for_method_calls"))
        .and_then(Value::as_str)
        .expect("workspace.lints.clippy.redundant_closure_for_method_calls is not configured");
