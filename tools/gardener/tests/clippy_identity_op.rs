...
#[test]
fn workspace_clippy_lint_configuration_enables_if_same_then_else_deny() {
    let mut manifest_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    manifest_path.pop();
    manifest_path.pop();
    manifest_path.push("Cargo.toml");
    ...
    let level = manifest
        ...
        .and_then(|workspace| workspace.get("lints"))
        .and_then(|lints| lints.get("clippy"))
        .and_then(|clippy| clippy.get("if_same_then_else"))
        .and_then(Value::as_str)
        .expect("workspace.lints.clippy.if_same_then_else is not configured");
...
#[test]
fn workspace_clippy_lint_configuration_enforces_manual_filter_deny() {
    let mut manifest_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    manifest_path.pop();
    manifest_path.pop();
    manifest_path.push("Cargo.toml");
...
#[test]
fn check_no_warnings_script_enforces_if_same_then_else_lint() {
    let manifest_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let mut script_path = manifest_path;
    script_path.pop();
    script_path.pop();
    ...
    assert!(
        script_text.contains("clippy::if_same_then_else"),
        "check-no-warnings.sh should explicitly pass clippy::if_same_then_else"
    );
}
...
#[test]
fn workspace_clippy_lint_configuration_enforces_manual_ok_or_deny() {
    let mut manifest_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    manifest_path.pop();
    manifest_path.pop();
    manifest_path.push("Cargo.toml");
...
#[test]
fn workspace_clippy_lint_configuration_enforces_manual_retain_deny() {
    let mut manifest_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    manifest_path.pop();
    manifest_path.pop();
    manifest_path.push("Cargo.toml");
...
fn workspace_clippy_lint_configuration_enables_needless_option_as_deref_deny() {
    ...
    let level = manifest
        ...
        .and_then(|workspace| workspace.get("lints"))
        .and_then(|lints| lints.get("clippy"))
        .and_then(|clippy| clippy.get("needless_option_as_deref"))
        .and_then(Value::as_str)
...
#[test]
fn workspace_clippy_lint_configuration_enables_unnecessary_to_owned_warn() {
    let mut manifest_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    manifest_path.pop();
    manifest_path.pop();
    manifest_path.push("Cargo.toml");
    ...
    let level = manifest
        ...
        .and_then(|workspace| workspace.get("lints"))
        .and_then(|lints| lints.get("clippy"))
        .and_then(|clippy| clippy.get("unnecessary_to_owned"))
        .and_then(Value::as_str)
        .expect("workspace.lints.clippy.unnecessary_to_owned is not configured");
