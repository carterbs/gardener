...
pub fn run_plan(ctx: &PlanContext<'_>) -> Result<PlanOutcome, GardenerError> {
    append_run_log("info", "plan_phase.started", json!({ "worker_id": ctx.identity.worker_id, "task_summary": ctx.task_summary }));
    if let Some(on_step) = ctx.on_step { on_step("PLAN", "starting planning phase"); }
    ...
    let result = run_agent_turn(AgentTurnInput {
        ...
        state: WorkerState::Planning, task_summary: ctx.task_summary,
        attempt_count: ctx.attempt_count, prompt_override: None, on_event: ctx.on_agent_event,
    })?;

    if result.terminal == AgentTerminal::Failure {
        let reason = result.payload.get("reason").and_then(|v| v.as_str()).unwrap_or("planning phase failed").to_string();
        return Err(GardenerError::Process(format!("planning phase agent failure: {reason}")));
    }

    if let Some(on_step) = ctx.on_step { on_step("PLAN", "planning phase complete"); }
    Ok(PlanOutcome { prompt_version: result.prompt_version, context_manifest_hash: result.context_manifest_hash })
}
