...
fn operator_keys_gated_without_flag() {
    for key in ['r', 'l', 'p'] {
        ...
        assert_eq!(
            action, None,
            "key '{key}' should be None without operator mode"
        );
    }
}
...
fn operator_keys_enabled_with_flag() {
    ...
    for (key, expected) in cases {
        ...
        assert_eq!(
            action,
            Some(expected),
            "key '{key}' should resolve to {expected:?} with operator mode"
        );
    }
}
...
fn back_action_available_always() {
    ...
    assert_eq!(
        action_for_key_with_mode('b', true),
        Some(HotkeyAction::Back)
    );
}
...
fn all_dashboard_bindings_have_matching_action() {
    for binding in &DASHBOARD_BINDINGS {
        ...
        assert!(
            action.is_some(),
            "dashboard binding '{}' ({}) has no matching action",
            binding.key,
            binding.action
        );
    }
...
fn all_operator_bindings_have_matching_action() {
    for binding in &OPERATOR_BINDINGS {
        ...
        assert!(
            action.is_some(),
            "operator binding '{}' ({}) has no matching action with operator=true",
            binding.key,
            binding.action
        );
    }
...
fn dashboard_controls_legend_includes_all_keys() {
    ...
    for binding in &DASHBOARD_BINDINGS {
        assert!(
            legend.contains(binding.key),
            "legend should contain key '{}': {legend}",
            binding.key
        );
    }
}
...
fn dashboard_controls_legend_with_operator_includes_operator_keys() {
    ...
    for binding in &OPERATOR_BINDINGS {
        assert!(
            legend.contains(binding.key),
            "operator legend should contain key '{}': {legend}",
            binding.key
        );
    }
}
...
fn report_controls_legend_includes_back_and_regenerate() {
    ...
    assert!(
        legend.contains('b'),
        "report legend should contain 'b': {legend}"
    );
    assert!(
        legend.contains('g'),
        "report legend should contain 'g': {legend}"
    );
}
